...
with chart1:
    st.header(f"График распределения сотрудников по количеству больничных дней :blue[старше {age}]", divider='blue')
    st.bar_chart(data=df_chart_old, x='Количество больничных дней', y='Количество сотрудников', height=500)

with chart2:
    st.header(f"График распределения сотрудников по количеству больничных дней :red[моложе {age}]", divider='red')
